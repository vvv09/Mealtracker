  ...
  <q-card>
    <q-card-section class="q-pa-xs text-center">
      <q-circular-progress
        ...
        class="q-ma-sm"
      >
        <q-icon color="protein" :name=Icons.PROTEIN size="lg"/>
      </q-circular-progress>
    </q-card-section>
...
<script setup lang="ts">
    ...
    const { calculatePortionProtein } = FoodCalculationUtils
    ...
    // Calories limit
    const percentage = computed(() => storeProfile.profile.proteinIntake ? (totalPerDate.value/storeProfile.profile.proteinIntake) * 100 : 0)
    const caloriesLimitHighlight = computed(() => {
      if (!storeProfile.profile.caloriesLimit) return 'positive'
      if (totalPerDate.value <= storeProfile.profile.proteinIntake) return 'positive'
      return 'negative'
    })
    const amountLeft = computed(() => storeProfile.profile.proteinIntake ? totalPerDate.value - storeProfile.profile.proteinIntake : 0)
