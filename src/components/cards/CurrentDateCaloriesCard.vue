  ...
  <q-card>
    <q-card-section class="q-pa-xs text-center">
      <q-circular-progress
        ...
        class="q-ma-sm"
      >
        <q-icon color="calories" :name=Icons.CALORIES size="lg"/>
      </q-circular-progress>
    </q-card-section>
      ...
      <q-item clickable class="q-pa-none">
        <q-item-section>
          ...
          <q-item-label caption><span 
            :class="amountLeft <= 0 ? 'text-positive' : 'text-negative'"
...
<script setup lang="ts">
    ...
    const { calculatePortionCalories } = FoodCalculationUtils
    ...
    // Calories limit
    const percentage = computed(() => storeProfile.profile.caloriesLimit ? (totalPerDate.value/storeProfile.profile.caloriesLimit) * 100 : 0)
    const caloriesLimitHighlight = computed(() => {
      if (!storeProfile.profile.caloriesLimit) return 'positive'
      if (totalPerDate.value <= storeProfile.profile.caloriesLimit) return 'positive'
      return 'negative'
    })
    const amountLeft = computed(() => storeProfile.profile.caloriesLimit ? totalPerDate.value - storeProfile.profile.caloriesLimit : 0)
